...
function update_yum() {
    func="update_yum"
    echo -e "\033[32m[1/$TOTAL_STEPS] Update yum source ...\033[0m"
    ...
    echo -e "\033[32m[1/$TOTAL_STEPS] Complete update yum source.\033[0m"
}

# 2. update gcc
function update_gcc() {
    func="update_gcc"
    echo -e "\033[32m[2/$TOTAL_STEPS] Update gcc ...\033[0m"
    ...
    if [ $? -eq 0 ]; then
        echo -e "\033[32m[2/$TOTAL_STEPS] Complete update gcc.\033[0m"
    else
        error "$func"
    fi
}
...
function update_libs() {
    func="update_libs"
    echo -e "\033[32m[3/$TOTAL_STEPS] Update dependent libraries ...\033[0m"
    # Note:
    ...
    $sshexec root:$root_passwd@$host:~ "$cmd" >>$log 2>&1
    if [ $? -eq 0 ]; then
        echo -e "\033[32m[3/$TOTAL_STEPS] Complete update dependent libraries.\033[0m"
    else
        error "$func"
    fi
...
function prerouting() {
    func="prerouting"
    echo -e "\033[32m[4/$TOTAL_STEPS] Pre Routing ...\033[0m"
    $sshexec -f $TOOLS_HOME/prerouting.sh -d \
        root:$root_passwd@$host:~ "sh prerouting.sh" >>$log 2>&1
    if [ $? -eq 0 ]; then
        echo -e "\033[32m[4/$TOTAL_STEPS] Complete pre routing.\033[0m"
    else
        error "$func"
    fi
}
...
function init_base_config() {
    echo "******************************************************"
    echo "   Initialize Base Configuration on $host "
    echo "******************************************************"
    update_yum
    update_gcc
    update_libs
