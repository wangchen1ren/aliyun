...
def init_machines(conf):
    ...
    for host in hosts:
        root_password = conf.get_password(host, 'root')
        script = os.path.join(NOAH_SHELL_DIR, 'machine_init.sh')
...
def gen_start_script(id, softs):
    ...
    start_script = os.path.join(instance_home, NOAH_START_SCRIPT_FILE_NAME)
    s = []
    s.append('#!/bin/sh')
    for soft in softs:
...
def gen_stop_script(id, softs):
    instance_home = os.path.join(INSTANCE_DIR, id)
    stop_script = os.path.join(instance_home, NOAH_STOP_SCRIPT_FILE_NAME)
    s = []
    s.append('#!/bin/sh')
    for soft in softs:
